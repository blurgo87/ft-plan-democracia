...
export default function Resultados({ data }: Props) {
    ...
    const informe = typeof rawInforme === 'string'
        ? stripThinkBlocks(
            (() => {
                const i = rawInforme.indexOf('# Informe Detallado');
                return i >= 0 ? rawInforme.slice(i) : rawInforme;
            })()
        ).trim()
        : '';
        ...
        <section className={rootClass}>
            ...
            {hasData && phase >= 3 && informe && (
                <div ref={informeRef} className={`${done ? '' : 'reveal-slide delay-2'} mt-4`}>
                    <div id="informe-detallado" className="border rounded-3 p-3 bg-white" style={{ borderColor: '#7bd5fe' }}>
                        <MarkdownTypeReveal
                            content={informe}
                            start={phase === 3}     // escribe SOLO en fase 3
                            speedChar={16}
                            pauseNewline={260}
            ...
            {hasData && phase >= 4 && todas.length > 0 && (
                <div
                    ref={tablaRef}
                    className={`animate__animated animate__slideInDown ${phase === 4 && !done ? 'animate__slower' : ''} mt-4`}
                    // O controla duración exacta:
                    style={{ ['--animate-duration' as any]: '2.2s' }} // ⇦ más lento
                    onAnimationEnd={() => {
                        if (phase === 4) {
                            if (hasLinks) setPhaseForward(5);
                            else {
                                setDone(true);
            ...
            {hasData && phase >= 5 && hasLinks && (
                <div
                    ref={enlacesRef}
                    className={`animate__animated animate__fadeInUp ${phase === 5 && !done ? 'animate__slower' : ''} mt-4`}
                    style={{ ['--animate-duration' as any]: '2.0s' }}
                    onAnimationEnd={() => {
                        if (phase === 5) {
                            setDone(true);
                            endRef.current?.scrollIntoView({ behavior: 'smooth', block: 'end' });
